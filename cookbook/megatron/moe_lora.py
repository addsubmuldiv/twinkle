...
parser.add_argument('--model', type=str, default='ms://Qwen/Qwen3-30B-A3B', 
...
def train():
    ...
    logger.info(f"Parallelism config: TP={TP_SIZE}, PP={PP_SIZE}, CP={CP_SIZE}, EP={EP_SIZE}, DP={DP_SIZE}")
    
    # Device mesh: Match Megatron's order "tp-cp-ep-dp-pp" from innermost to outermost
    # Shape: (PP, DP, EP, CP, TP)
    device_mesh = DeviceMesh(
        device_type='cuda',
        mesh=np.arange(WORLD_SIZE).reshape(PP_SIZE, DP_SIZE, EP_SIZE, CP_SIZE, TP_SIZE),

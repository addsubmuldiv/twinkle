...
def eval(model: TransformersModel):
    dataloader = DataLoader(dataset=partial(create_dataset, data_slice=range(100)), batch_size=8)
...
def train():
    dataloader = DataLoader(dataset=partial(create_dataset, data_slice=range(2000)), batch_size=8, remote_group='model')

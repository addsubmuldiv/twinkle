...
def eval(model: TransformersModel):
    dataset = Dataset(dataset_meta=DatasetMeta('ms://swift/self-cognition', data_slice=slice(0, 5000)))
...
def train():
    ...
    for step, batch in enumerate(dataloader):
        ...
        if step % 16 == 0:
            logger.info(f'Current is step {step // 16}, metric: {model.calculate_metric()}')

...
def process_example(example: dict, tokenizer) -> types.Datum:
    ...
    target_tokens = tokens[
        1:]  # We're predicting the next token, so targets need to be shifted.
    weights = weights[1:]
...
for i, (inp, tgt, wgt) in enumerate(
        zip(datum0.model_input.to_ints(),
            datum0.loss_fn_inputs['target_tokens'].tolist(),
            datum0.loss_fn_inputs['weights'].tolist())):
    print(
        f"{repr(tokenizer.decode([inp])):<20} {repr(tokenizer.decode([tgt])):<20} {wgt:<10}"
    )
...
for epoch in range(2):
    for batch in range(5):
        ...
        print(
            f"Loss per token: {-np.dot(logprobs, weights) / weights.sum():.4f}"
        )
...
params = types.SamplingParams(max_tokens=20, temperature=0.0,
                              stop=['\n'])  # Greedy sampling
future = sampling_client.sample(prompt=prompt,

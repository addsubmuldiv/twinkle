...
def build_model_app(nproc_per_node: int,
    ...
    class ModelManagement():
        ...
        def __init__(self, nproc_per_node: int, device_group: Dict[str, Any], device_mesh: Dict[str, Any], **kwargs):
            ...
            twinkle.initialize(mode='ray', nproc_per_node=nproc_per_node, groups=[
                               self.device_group], lazy_collect=False)
            ...
            async def _load_model():
                ...
                if body.lora_config:
                    ...
                    self.model.set_processor(
                        'InputProcessor', adapter_name=adapter_name)
                    self.model.set_loss('CrossEntropyLoss',
                                        adapter_name=adapter_name)
                    self.model.set_optimizer(
                        'AdamW', adapter_name=adapter_name)
        ...
        async def get_info(self, request: Request, body: types.GetInfoRequest) -> types.GetInfoResponse:
            ...
            model_name = metadata.get(
                "base_model") if metadata else str(body.model_id)
            lora_rank = None
            is_lora = False
        ...
        async def forward(self, request: Request, body: types.ForwardRequest) -> types.UntypedAPIFuture:
            async def _do_forward():
                ...
                try:
                    ...
                    output = self.model.forward_only(
                        inputs=datum_list, adapter_name=adapter_name)
                    loss = self.model.calculate_loss(
                        adapter_name=adapter_name, **loss_fn_config)
        ...
        async def save_weights(self, request: Request, body: types.SaveWeightsRequest) -> types.UntypedAPIFuture:
            async def _do_save():
                try:
                    ...
                    self.model.save(
                        name=name, output_dir=save_path.as_posix(), adapter_name=adapter_name)

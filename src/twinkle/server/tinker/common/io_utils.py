...
class CheckpointManager:
    ...
    @staticmethod
    def get_save_dir(model_id: str, name: str, is_sampler=False) -> str:
    ...
    @classmethod
    def save(cls, model_id: str, name: str, is_sampler=False):
        weights_type = 'sampler_weights' if is_sampler else 'weights'
        checkpoint_type = 'sampler' if is_sampler else 'training'
        checkpoint_id = f'{weights_type}/{name}'
        tinker_path=f"twinkle://{(model_id / checkpoint_id).as_posix()}"
    ...
    def get_weights_info(
        ...
        # This seems to be for model weight files, potentially distinct from our metadata wrapper
        # But if stored in same dir:
        metadata_path = Path(checkpoint_path) / CHECKPOINT_INFO_FILENAME
        if not metadata_path.exists():
            return None
        try:
            with open(metadata_path, 'r') as f:
                data = json.load(f)
                return types.WeightsInfoResponse(**data)
        except Exception:
            return None
